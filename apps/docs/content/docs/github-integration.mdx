...
| Auto-link PRs | Links a PR to the issue whose identifier is in its branch name or title. |
...
The PR body isn't scanned, and keywords carry no special meaning: `Closes MUL-123` or `Related to MUL-123` in the body links nothing. Commit messages and PR comments don't trigger linking either.
...
Under the list, one line says what happens next — for example *Completes when #19 merges*, or which PR closed without merging.
...
With **Complete issues when their PRs merge** on (the default), an issue moves to `Done` when every PR linked to it is merged:

1. the issue has at least one linked PR, and every linked PR is `Merged` — an `Open` or `Draft` PR keeps it waiting, and so does a PR closed without merging until you remove it from the issue;
2. the issue isn't `done` or `cancelled`, and auto-complete hasn't been turned off for it (**Pull requests → ⋯ → Don't auto-complete this issue**), for work that needs review or a release after the merge.
...
- **Identifier only in the body**: put it in the branch name or PR title, or link the PR by hand.
...
- **Issue not completed after the PR merged**: read the line under the issue's **Pull requests** list. It names any PR still open or closed without merging, or says auto-complete is off for the issue or the workspace.
