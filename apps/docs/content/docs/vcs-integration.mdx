...
description: Connect a self-hosted Forgejo, Gitea, or GitLab instance per workspace so pull/merge requests that carry an issue identifier in their branch name or title auto-attach to that issue, move it to Done once every linked PR is merged, and show CI status.
---
...
- **Issue links** — an identifier in the title or branch links the PR to the issue; the body isn't scanned, so `Closes MUL-123` there links nothing. Once every PR linked to the issue is merged — across GitHub and self-hosted providers — the issue moves to **Done**, under the same workspace setting and rules as [GitHub](/github-integration).
