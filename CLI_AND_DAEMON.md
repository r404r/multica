...
This disables periodic and server-requested CLI self-update and the on-disk
binary auto-reload described above, and prevents the daemon from handing its
lifecycle to a successor process. Upgrade the managed binary explicitly, then
restart it through the external supervisor.
