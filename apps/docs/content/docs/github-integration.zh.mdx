...
| 自动关联 PR | 分支名或标题含任务编号的 PR，会关联到对应任务。 |
| **PR 卡片 → CI 与可合并性** | 对每个关联的 PR，Multica 从 GitHub API 拉取一份经鉴权的快照，把它的 **CI 状态**和**可合并性**镜像到卡片上（见下方 **PR 卡片展示什么**）。 |



PR 合并后是否自动完成任务，是另一项工作区设置：**设置 → 任务状态 → 自动化 → PR 合并后自动完成任务**。它对所有代码托管平台生效，包括[自托管 Git 代码托管](/vcs-integration)。
...
Multica 不读取 PR 正文，关键词也没有特殊含义：正文里写 `Closes MUL-123` 或 `Related to MUL-123` 都不会建立关联。Commit message 和 PR 评论也不会触发关联。
...
列表下方有一行说明接下来会发生什么，例如「等待 #19 合并后自动完成」，或者哪个 PR 已关闭、未合并。

点击记录可以打开 GitHub 上的 PR。关闭 **PR 侧栏** 只会隐藏这个区块，不会断开连接。

## PR 合并转 Done 的条件

开启 **PR 合并后自动完成任务**（默认开启）时，任务的所有关联 PR 都合并后，任务改为 `Done`：

1. 任务至少有一个关联 PR，且所有关联 PR 都是 `Merged`。仍为 `Open` 或 `Draft` 的 PR 会让任务继续等待；关闭但未合并的 PR 也一样，直到你把它从任务上移除；
2. 任务当前不是 `done` 或 `cancelled`，也没有单独关闭自动完成（**Pull requests → ⋯ → 此任务不自动完成**），适用于合并后还需要验收或发布的工作。

只有 PR 事件会触发判断：关联 PR 合并、新增关联或移除关联。重新打开任务后，它不会被改回 `Done`，下一个关联 PR 合并时才会。开启这项设置，也不会补完成此前已合并的任务。
...
- **编号只写在正文里**：把编号放到分支名或 PR 标题中，或者手动关联。
...
- **PR 合并后任务没完成**：看任务 **Pull requests** 列表下方的说明，它会指出仍未合并或已关闭未合并的 PR，或者说明任务、工作区关闭了自动完成。
