...
| PR 자동 연결 | 브랜치 이름이나 제목에 태스크 번호가 있는 PR을 해당 태스크에 연결합니다. |
...
**PR 병합 시 태스크 자동 완료**(**설정 → 태스크 상태 → 자동화**, 기본값 켜짐)가 켜져 있으면 태스크에 연결된 PR이 모두 merge될 때 태스크가 `Done`으로 바뀝니다.

1. 태스크에 연결된 PR이 하나 이상 있고 모두 `Merged` 상태입니다. `Open`이나 `Draft` PR이 남아 있으면 태스크는 계속 기다립니다. merge되지 않고 닫힌 PR도 태스크에서 제거할 때까지 마찬가지입니다.
2. 태스크가 `done` 또는 `cancelled`가 아니고, 이 태스크만 자동 완료를 끄지 않았습니다(**Pull requests → ⋯ → 이 태스크는 자동 완료하지 않음**). merge 후에 리뷰나 릴리스가 필요한 작업에 사용합니다.

판단은 PR 이벤트가 태스크에 닿을 때만 이루어집니다. 연결된 PR의 merge, 연결 추가, 연결 제거입니다. 태스크를 다시 열어도 `Done`으로 돌아가지 않으며, 다음에 연결된 PR이 merge될 때 완료됩니다. 설정을 켜도 이전에 merge된 태스크를 소급해 완료하지 않습니다.

상태 변경은 관련 PR을 적은 시스템 작업으로 타임라인에 기록되고 해당 태스크 구독자에게 알림이 전달됩니다. 이 설정은 [자체 호스팅 Git 코드 호스팅](/vcs-integration)을 포함한 모든 코드 호스트에 공통으로 적용됩니다.
...
- **PR merge 후 태스크가 완료되지 않음**: 태스크의 **Pull requests** 목록 아래 설명을 확인합니다. 아직 merge되지 않았거나 merge 없이 닫힌 PR, 또는 태스크나 워크스페이스에서 자동 완료가 꺼져 있음을 알려 줍니다.
